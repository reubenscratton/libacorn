...
acorn::string acorn::operator "" _S(const char *str, std::size_t len) {
    return acorn::string::static_str(str, (int32_t)len);
}
...
string acorn::operator+(const string& lhs, const string& rhs) {
    string r(lhs);
    r.append(rhs);
    return r;
}
