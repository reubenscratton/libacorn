...
struct null_t {};
inline constexpr bool operator<(null_t, null_t) noexcept { return false; }
inline constexpr bool operator>(null_t, null_t) noexcept { return false; }
inline constexpr bool operator<=(null_t, null_t) noexcept { return true; }
inline constexpr bool operator>=(null_t, null_t) noexcept { return true; }
inline constexpr bool operator==(null_t, null_t) noexcept { return true; }
inline constexpr bool operator!=(null_t, null_t) noexcept { return false; }

/*template <>
struct std::hash<null_t> {
  using argument_type = null_t;
  using result_type = size_t;
  inline
  result_type operator()(const argument_type&) const _NOEXCEPT {
    return 66740832; // return a fundamentally attractive random value.
  }
};*/

// RJS mod: This is to support a 'null' option in std::variant -backed JS values.
// We basically map the JS value 'null' to null_t.
template <>
struct js_traits<null_t>
{
    static JSValue wrap(JSContext * ctx, const null_t) noexcept
    {
        return JS_NULL;
    }

    static null_t unwrap(JSContext * ctx, JSValueConst v)
    {
        return null_t{};
    }
};
...
struct js_traits<std::variant<Ts...>>
{
    ...
    template <typename T> struct is_string
    {
        static constexpr bool value = std::is_same_v<T, const char *> ||
            ...
            std::is_same_v<std::decay_t<T>, acorn::string> ||
    ...
    static std::variant<Ts...> unwrap(JSContext * ctx, JSValueConst v)
    {
        ...
        switch(tag)
        {
            ...
            // RJS mod: it is useful to allow variant properties to be null
            case JS_TAG_NULL:
                return std::variant<Ts...> { null_t() };
